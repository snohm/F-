...
Idea: test imperative queues against a (functional) **model** by
abstracting the queue operation into abstract versions that can be
generated. It requires these steps:
...
3. Execute commands on the implementation [function run_cmd]
    ...
    - if the command does an obervations, it must be the same in the
...
success. Both get and top perform the corresponding queue command and
compare the result with the front of the list.  Either of get , top ,
or List.head can raise an exception,  and this is why we have a pre condition:*)

let run_cmd c s q = 
    match c with
    | Pop ->
        if pre_cond s q then  get q = List.head s else raise EmptyQueue
    | Top ->
        if pre_cond s q then  top q = List.head s else raise EmptyQueue
    | Put n -> put n q; true


(* a recursive agreement checker that walks the list of symbolic
commands: *)
